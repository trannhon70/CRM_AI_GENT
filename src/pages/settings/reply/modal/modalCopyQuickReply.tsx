...
const ModalCopyQuickReply: FC<IProps> = (props) => {
    ...
    const getDataCache = async () => {
        const result: any = await cacheRepository.get("getFanPagesId");
    ...
    const handleCopy = () => {
        ...
        if (!form.landing_id) {
            toast.warning("Vui lòng chọn trang đích để sao chép.");
            return;
        }
        setLoadingCopy(true);
                ...
                <Box sx={style} className="flex flex-col ">
                    ...
                    <div className='flex flex-col flex-1 min-h-0 gap-2.5 p-3 bg-[#F2F4F7]' >
                        <div className='flex items-center gap-5 shrink-0 ' >
                            ...
                            <ComponentSelect source_page={source_page} onSelect={handleOnSelect} />
                        </div>
                        ...
                        <div className='flex flex-col flex-1 min-h-0 bg-white rounded p-3 ' >
                            <div className='flex items-center justify-between shrink-0' >
                                ...
                                />
                                {
                                    form.selectedKeys.length > 0 && <Chip variant="filled" color="primary" label={`Đã chọn ${form.selectedKeys.length || 0}`} size="small" />
                                }
                    ...
                    <div className='flex items-center justify-between h-14 px-7 gap-2.5 bg-white rounded-bl rounded-br' >
                        <FormControl>
                            <RadioGroup
                                ...
                                value={form.mode}
                                onChange={(e) => setForm((prev: any) => ({ ...prev, mode: e.target.value as 'append' | 'replace' }))}
                            >
                        ...
                        <div className='flex items-center justify-end gap-2.5' >
                            ...
                            <Button loading={loadingCopy} onClick={handleCopy} variant="contained" sx={{ height: 35, px: 2, textTransform: "none" }}>Sao chép</Button>
                        </div>
                    </div>
                </Box>
            </Fade>
