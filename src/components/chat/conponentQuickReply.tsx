...
import React, { Fragment, useLayoutEffect, useMemo, type FC } from "react";
import { FaEdit } from "react-icons/fa";
import { IoChatbubbleEllipsesOutline, IoSendSharp } from "react-icons/io5";
...
const ComponentQuickReply: FC<IProps> = (props) => {
    ...
    const categoryOptions = useMemo(() => {
        const realOptions = Array.from(
            new Map(
                dataAll
                    .filter((i) => i.quickReplyCategory)
                    .map((i) => [i.quickReplyCategory.id, i.quickReplyCategory])
            ).values()
        ).map((c) => ({ value: c.id, label: c.name }));

        const hasNullItem = dataAll.some((i) => !i.quickReplyCategory);

        return hasNullItem
            ? [...realOptions, { value: NULL_CATEGORY_VALUE, label: "Chưa có chủ đề" }]
            : realOptions;
    }, [dataAll]);
    ...
    const filteredData = useMemo(() => {
        if (categoryFilter.length === 0) return dataAll;
        return dataAll.filter((item) => {
            const catId = item.quickReplyCategory?.id ?? NULL_CATEGORY_VALUE;
            return categoryFilter.includes(catId);
        });
    }, [dataAll, categoryFilter]);
        ...
        >
            <div className="flex flex-col h-full">
                <div className="py-1 px-2 shrink-0 border-b-gray-300 border-b" >
                    ...
                    <div className="flex items-center justify-end text-blue-900 font-medium gap-2.5 " >
                        Mẫu trả lời nhanh
                        <IconActionButton
                            icon={<FaEdit size={20} />}
